...
- **How the runner presents it has not changed.** The header, on every request the capture page makes to our own origin and on nothing else — section 11 has why that boundary is the origin rather than the document, and bug 46 is what a tighter one cost. `scripts/render-tape.mjs` reads `RENDER_KEY` from the shell first and `.dev.vars` second exactly as before; what goes in that variable is a minted key rather than the Worker's secret.
...
**It needs `RENDER_KEY` as well as `CLOUDFLARE_API_TOKEN`.** The capture page it screenshots is not public — section 6c, and [DEPLOY.md](DEPLOY.md#video-rendering) for the operational half. Without the key the script says so before it launches Chrome, along with ffmpeg being absent and Chrome being somewhere other than where it looked.

**The key goes out on every request that is same-origin with `--base`, and on nothing else.** It used to be set with `setExtraHTTPHeaders`, which put a secret that can read any card on the instance onto every photograph, font and chunk the page fetched — including, for a backdrop or a sponsor mark hosted elsewhere, a request to somebody else's server. Narrowing that to the document alone was one boundary too far and it broke every draft render: `/media` asks the same question the capture page does (section 6d), so on an unpublished show the page's own portraits came back 404 and the bout rendered with nobody in it (bug 46). Origin is the line the concern actually drew — the third-party host is the thing to keep the key away from, and our own routes are the things that check it — so `sameOrigin` in [scripts/render-tape.mjs](scripts/render-tape.mjs) decides it by `URL.origin` rather than by a prefix match on the base, because `https://eventiq.win` is a prefix of `https://eventiq.win.example.com` and the same host over http is somewhere a secret must not go in the clear. It does not depend on publication, and must not: the runner renders drafts and published cards in one pass, and one rule for both is the argument section 6c makes about the capture page itself.
...
**A finished row is claimable when what it published is not what the bout hashes to now**, and it was not until the first run from CI said so. That run rendered bouts 1 to 10 of the demo card, which have no `render_jobs` row at all, and printed `bout 15: left to another runner (stale)` for each of the five that do — the renders committed under `public/` before there was a bucket, seeded as `done` with a `current_r2_key` and, at the time those rows were written, no `current_hash`. `--stale` selected all five correctly; the claim then refused them, because it took only a row saying `queued`, or one saying `failed` or `running` with attempts left. So the message was untrue twice over — nobody held those bouts, and no run would ever take them — and the five videos the whole pitch is built on were the only ones on the card that could not be remade without an operator typing a bout number. The claim now also takes a `done` row whose `current_hash` `IS NOT` the fingerprint it is claiming with, which is the same question `renderState` answers with the word "stale"; `IS NOT` rather than `<>` because `<>` against a NULL hash is NULL rather than true, and a render that predates fingerprinting is exactly that row. The lease is still tested first and separately, so a bout another runner holds is left alone however stale it looks, and `claimable` in [lib/renders.ts](lib/renders.ts) takes the live hash as an argument now so the readable half and the SQL still decide the same thing — [lib/renders.test.ts](lib/renders.test.ts) holds the two together by asserting that a run takes exactly the finished rows the dashboard calls stale.
...
- **The same gate, through the same function.** The capture page loads its card with `loadRenderableCard`, exactly as the bout one does and for the reason section 6c gives. An unknown template id, a show that does not exist and a caller who has proved nothing all answer 404.
- **The clock is a prop.** "N days to go" is worked out from a `now` the page fixes once, not from `Date.now()` inside the composition: a composition that reads the clock is not a pure function of its props, and the count would tick over partway through a capture with nothing to say so. `daysToGoLabel` in [lib/promo.ts](lib/promo.ts) also refuses to print a nought, which is the zero-bout rule from section 14 applied to a countdown.
- **A card is between one bout and fifteen.** `cardRowFrames` divides the scroll's budget by the number of rows and holds the result between a floor and a ceiling, so a full bill fits and a six-bout card is not three seconds of list followed by nine of nothing.
- **No `--publish`, and no bucket.** Nothing on the site plays these, so there is no `render_jobs` row, no fingerprint and no lease — which is why [scripts/render-show.mjs](scripts/render-show.mjs) is a fraction of the bout renderer. It duplicates that file's same-origin rule, Chrome flags, frame loop and ffmpeg arguments rather than importing them, because `render-tape.mjs` reads its base, its slug and its environment from `process.argv` at module scope and importing it would put one script's command line in charge of the other's state. An encode change has to be made in both, and both say so.
...
### From the walkthrough on staging

One, and it is the argument for running the walkthrough against a deployed Worker rather than a dev server. Nothing local could have seen it, because the thing that was wrong only exists where there is https.

44. **Signing out did nothing whatsoever anywhere there was https to do it over.** The session cookie takes a `__Host-` prefix off localhost (bug 37), and that prefix is not a naming convention: a browser checks `Secure`, `Path=/` and the absence of a `Domain` on every Set-Cookie arriving under that name, and discards the whole header if one of them is missing. Expiring a cookie *is* a Set-Cookie — an empty value with a dead expiry — so it is checked the same way, and `cookies().delete(name)` with a bare name does not survive the check. Next turns that call into `set({ ...options, name, value: "", expires: new Date(0) })`, and with a string there are no options, so what went out was `__Host-eventiq_session=; Path=/; Expires=Thu, 01 Jan 1970 00:00:00 GMT` with no `Secure` on it. The browser threw it away, kept the cookie it already had, and the promoter who had just clicked "Sign out" and been sent to the front page could walk straight back into the dashboard. On localhost the name is the plain `eventiq_session`, no prefix rules apply, the bare deletion is honoured, and every local run of the same walkthrough passed. `sessionCookieOptions` in [lib/auth.ts](lib/auth.ts) now holds the attributes beside the name and `SESSION_COOKIES` pairs each name with the ones that name demands; setting and clearing both go through it, so there is nothing left for the two to disagree about. The suite was changed as well as the code — the cookie jar in tests/db records what a Set-Cookie would carry rather than only whether the jar emptied, because a jar that forgets a cookie on request agrees with any deletion anybody cares to write. **An attribute the browser validates is part of the cookie’s identity, not decoration**, and a removal is a write, so it has to be spelled exactly the way the write was.
45. **A promoter with one show could never make a second.** The new-show form lived on the promoter's index, and the index sends a promoter with exactly one show straight to that show — the right thing for the commonest case, and it meant the form was unreachable from the moment the first show existed. Found by trying to load a real card for a real night onto the account that already held the demo. The form has an address of its own now, /promoter/new, and the dashboard links to it. **A page that redirects on the common case needs another way to reach what it was hiding.**

### From the first draft card the runner rendered

One, and it is bug 27's argument one level down. Read the two together: the same rule, the same reasoning about why this one is different, and this time the exception was made in the client rather than in the route.

46. **Every video the runner made for an unpublished show came out with no fighters in it.** Six bouts of `budo-fighting-championship`, every fighter with a photograph and a cutout in the bucket, rendered at full length with the name plates, the glow and the backdrop all correct, an empty space where the subject belongs and a broken-image marker in the top left of all 480 frames. Every part of the pipeline reported success: the mp4 was the right length, the job went to `done`, the exit code was 0.

    Two things had to be true at once. The first is that `/media` refuses a draft show's objects to a request with no render key on it, which is section 6d and is correct — the objects belong to the card, and the card is not public yet. The second is that the renderer had stopped sending the key with them. It had been on every request the page made, which put a secret that reads any card on the instance onto anything the page fetched from anywhere, so it was narrowed to the capture page's own document. That reasoning was right about the danger and wrong about where the line goes: **the page's own `<img src="/media/...">` is a request to the route that checks the key**, and it went out without one, and came back 404. On the published demo card the objects are public, so every render anybody had watched was of the one card where the key made no difference — the same shape as bug 30, where the seeded cutouts hid a broken pipeline for weeks.

    The line is the origin now. `sameOrigin(url, base)` in [scripts/render-tape.mjs](scripts/render-tape.mjs) decides it for the document and every subresource alike, by `URL.origin` rather than by a prefix match on the base URL, and a sponsor mark on somebody else's host still gets nothing — which was the whole of the original concern. It cannot depend on publication, because the hourly runner takes whatever is queued and does not know whose card it will be.

    **What made this survivable is the more important half.** A refused image is not a missing image as far as the DOM is concerned: `img.complete` is true of a picture that arrived and true of one that 404ed, and the capture page signalled `window.__ready` on exactly that. Then the exporter's own `decode()` — added by bug 31 to make sure a newly mounted portrait had painted before it was photographed — put its rejection into an empty `catch`, which is the one place the truth was available and the one place it was discarded. So the failure had to be seen by a person opening the file. Both are closed. `captureState` in [lib/capture.ts](lib/capture.ts) is the pure rule — a finished request with no intrinsic width is a broken image, and it is a pure function of a list of image states so it can be tested without a browser — `RenderStage` publishes what it finds as `window.__brokenImages`, and the renderer refuses to capture a bout that reports any, failing the job with the first broken URL in the message. It asks on every frame, because a portrait does not exist in the document until its reveal starts at frame 62, so the answer at frame 0 is an answer about the backdrop. `TaleOfTheTape` is untouched and stays a pure function of its props; all of this is in the stateful wrapper around it.
