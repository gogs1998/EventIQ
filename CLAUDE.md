...
5. **Put `RENDER_KEY` into the repository secrets, and somewhere a person can read it.** The hourly render workflow needs it, along with `CLOUDFLARE_API_TOKEN` and `CLOUDFLARE_ACCOUNT_ID`, and until all three are set that workflow fails every hour. It is set on the Worker, cannot be read back, and whoever wants to render mints a fresh one — which was fine while one person rendered on their own laptop and is not now that a schedule needs it. A password manager entry as well as the secret, and note that anyone who can push a workflow can read it.
...
13. ~~**Render on a schedule.**~~ Done: an hourly GitHub workflow runs `--stale --publish --remote` for every published show, and the dashboard can queue a bout. Still Chrome and ffmpeg on a machine that is not Cloudflare; no longer the laptop. HANDOVER section 11.
14. **Render the remaining ten bouts** so no bout in the demo is a dead end. The hourly workflow does this on its own once the secrets are set.
