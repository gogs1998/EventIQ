 ...
 *
 * "New link" is the one control here that cannot be allowed to fail silently: a
 * promoter who presses it and sees nothing will send the old link, and the old
 * link is the one they pressed it to stop working.
 */
export function InviteLink({
  ...
  const copy = async () => {
    ...
    if (!sent) {
      start(async () => {
        const result = await markInviteSent(slug, fighterId);
